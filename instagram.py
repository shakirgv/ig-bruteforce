...
class Engine(object):
    ...
    def get_user_resp(self):
        return self.display.prompt(
            "Hücumu davam etdirmek isteyirsiniz? [y/N]: "
        )
...
def valid_int(n):
    if not n.isdigit():
        raise ArgumentTypeError("mod nömre olmalıdır")

    n = int(n)

    if n > 3:
        raise ArgumentTypeError("mod üçün maksimum 3-dür")

    if n < 0:
        raise ArgumentTypeError("mod üçün minimum 0-dır")
...
def valid_float(n):
    ...
    err_msg = "prune 0 ile 1 arasında bir deyer olmalıdır"
...
def args():
    args = ArgumentParser()
    args.add_argument("-u", "--username", help="email ve yaxud istifadeçi adı")
    args.add_argument("-p", "--passlist", help="şifre siyahısı")
    args.add_argument("-px", "--proxylist", help="proxy siyahısı")
    args.add_argument(
        "--prune",
        default=-1,
        type=valid_float,
...
if __name__ == "__main__":
    ...
    if int(python_version()[0]) < 3:
        print("[!] Zehmet olmasa Python 3 versiyasından istifade edin!")
        exit()
